...
STRATEGIES: List[Dict[str, Any]] = [
    {'id': 'opening_breakout', 'name': 'Opening Breakout', 'fn': _s_opening_breakout,
     'description': 'Buys a confirmed, volume-backed hold above the opening range while it is still fresh.',
...
def evaluate_all_strategies(features: Dict[str, Any], settings: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Run all 10 strategies against one stock's features. Every strategy is always
    evaluated (never short-circuited) so the decision log can show what each one
    thought, including the ones that declined -- that's what makes "why did it NOT
    take strategy X?" answerable after the fact."""
    out: List[Dict[str, Any]] = []
    for spec in STRATEGIES:
        try:
            sig = spec['fn'](features, settings or {})
            out.append(sig.to_dict())
        except Exception as e:  # a broken strategy must never take the scan down
            out.append({'strategy_id': spec['id'], 'name': spec['name'], 'eligible': False,
                        'confidence': 0.0, 'reasons': [], 'blocks': [f'evaluation_error: {e}'],
...
def select_strategy(evaluations: List[Dict[str, Any]], regime: str,
    ...
    for ev in evaluations:
        ...
        ranked.append({
            'strategy_id': sid, 'name': ev.get('name'),
    ...
    for r in ranked:
        ...
        if ev_rejected_here:
            ev_rejected.append({'strategy_id': sid, 'name': r['name'],
