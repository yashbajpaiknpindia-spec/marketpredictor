# v3.2.5 historical export update

See [V3.2.5_DEPLOYMENT.md](V3.2.5_DEPLOYMENT.md) for the memory fixes, measured stress results, and Render deployment steps. Earlier version audits below are historical records.
...
## V3.2.1 Opportunity / Barrier / Parity Fixes

`v3.2.1-opportunity-barrier-parity` is the post-forensic-fix build for the NSE/BSE cash-equity intraday engine. It is not a simulation result.

Fixed in the application/research contract:
- no universal 11:20 candidate blackout; opening families have family-specific warm-up only;
- fractional percentage return units are used consistently;
- LONG and SHORT share the same strategy family library, EV evidence rules, risk plan, order semantics and exit logic;
- VWAP reclaim compares each historical close to its matching historical VWAP;
- target/stop and P&L calculations are direction-aware;
- brokerage uses the configured lower-of-rate/cap rule and one canonical cost model;
- the reported economic hurdle is the actual acceptance condition, including execution stress;
- the old tiny-sample family validation gate is diagnostic only and cannot manufacture a zero-trade result;
- portfolio research ranks candidates by predicted net edge at each timestamp and uses the simulated barrier exit time for concurrency;
- historical-barrier labels are aligned with target-before-stop, MFE, MAE and time-to-event objectives;
- paper positions can be created for either long or short orders;
- live routing remains hard-blocked while production authorization is false;
- post-fix auditing is separate from simulation and must pass before research is run.

The verified historical corpus remains 5-minute-only. Genuine historical 1-minute, bid/ask, tick/order-book and point-in-time membership data are still required for stronger execution/production validation.
...
# Update: position limits, capital reporting, and mode-separated results

Four changes, plus two bugs found while making them.

## 1. Max intraday positions is now a real, visible setting

**The problem.** The intraday engine had no position limit of its own. It silently used Trading Automation's global `max_open_positions`, which defaults to **3**. So intraday was capped at three concurrent positions, the number appeared nowhere on the Intraday Engine page, and changing it would also have changed swing/delivery trading.

**Now.** `max_open_positions` is an intraday-owned setting with its own database column and its own field on the Intraday Engine page.

- Default: **50**
- **0 = unlimited**
- Maximum accepted: `INTRADAY_POSITION_LIMIT_MAX` (default 1000)
- Trading Automation keeps its own separate limit; the Position slots status card shows both so the two are never confused.

The `0 = unlimited` convention is interpreted in exactly one place, `resolve_position_limit()`, so "unlimited" can never be misread anywhere as "zero free slots" (which would silently block every entry). `available_position_slots()` returns a large finite integer for unlimited, so all the existing `slots -= 1` arithmetic keeps working unchanged.

### Bug found: the trades/day setting was un-saving itself

`init_db()` ran this on **every** application start:

```sql
UPDATE intraday_settings SET max_trades_per_day = 3 WHERE max_trades_per_day = 50;
```

Any time you set the intraday trade cap to 50, the next restart or redeploy quietly pulled it back to 3, with no message anywhere. A saved setting that un-saved itself. That line has been removed — 50 is now the intended default.

The per-day trade cap was also raised from a hard ceiling of 50 to 1000, because a 50-position limit combined with a 3-trade/day limit is really a 3-trade system, and the trades/day cap would otherwise be the invisible constraint.

### One-time floor migration

`ALTER TABLE ... DEFAULT` only affects rows inserted afterwards, never rows that already exist — so on an existing install the new default would have looked like it did nothing. A one-time migration raises `max_open_positions` and `max_trades_per_day` to at least `INTRADAY_POSITION_LIMIT_FLOOR` (50). It is guarded by its own `position_limits_floor_applied` column, so it runs exactly once and never overrides a lower value you deliberately set afterwards.

## 2. Intraday Replay defaults to unlimited positions

Research should measure every signal the strategy produced. A position cap during replay does not just reduce the trade count — it biases it: whichever signals arrive earliest in the day fill the slots, and every later signal, **including the better ones**, is discarded before it can be measured.

The Replay Lab now has a **Max open positions for this replay** control:

- **Unlimited** — the default
- **Match the intraday engine's setting**
- **Specific number…**

The resolved limit is stamped onto the run's settings snapshot at start time, so a finished run always reports the limit it actually enforced rather than re-deriving it from whatever the settings say later.

### Capital guard: unlimited positions, finite money

Unlimited positions only makes sense as research if money stays finite. Each position is sized off full capital, so without a cash constraint a ₹2,00,000 account would report ₹40,00,000 "invested" — a number the account could never have funded, which makes every capital figure meaningless.

The **capital guard** (on by default) makes positions unlimited in *number* while capping the total value of open positions at the run's capital. Capital is released and reused when a position closes, exactly as a real account recycles money through the session. A refused signal is logged explicitly:

```
capital_fully_deployed: needs ₹48,000, only ₹12,400 of ₹2,00,000 free
(4 position(s) already holding ₹1,87,600)
```

Turn it off only for notional "what did the strategy find" counts. When it is off, the breakdown flags the over-deployment rather than hiding it.

## 3. Capital & P&L breakdown

New card on the Intraday Engine page, and a new endpoint:

```
GET /api/capital-report?source=all&capital=200000&start=2026-09-01&end=2026-09-15
```

Give it a starting capital and a period, and it reports:

| | |
|---|---|
| **Started with** | your capital, and the number of closed trades/days |
| **Amount invested** | total turnover, average per trade, turnover as a multiple of capital |
| **Peak money in use** | the most money held in open positions at any single moment, plus how much stayed idle |
| **Gross P&L** | before any costs |
| **Total costs** | brokerage, STT, exchange/SEBI, GST, stamp duty, slippage — per trade and as a % of gross |
| **Net P&L** | after every charge, with return on capital |
| **Ending capital** | what you are left with |
| Trade quality | win rate, avg win/loss, largest win/loss, profit factor, expectancy, max drawdown |

Plus a **day-by-day equity curve** (opening capital → invested → gross → costs → net → closing capital), and attribution **by strategy** and **by exit reason** (worst first — usually where the money actually leaks).

Two distinct "invested" figures are reported on purpose. **Total invested** is turnover and double-counts capital recycled through the day; **peak deployed** is computed by a sweep over entry/exit timestamps and is the figure that decides whether the period was fundable with a given account. Turnover alone can suggest you needed several times more money than you did.

If peak deployed exceeds starting capital, the report says so explicitly rather than presenting an unfundable result as if it were achievable.

There is also a **Capital & P&L breakdown for this run** button in the Replay Lab, which scopes the same report to one replay run using that run's own capital.

## 4. Strategy Dashboard no longer mixes execution modes

**The problem.** `/api/intraday/strategies` queried `intraday_replay_trades` and nothing else. Paper and live results were not merely mixed in — they were **completely absent**. A strategy could have a long live record and still display as "no trades recorded yet", and a replay-only record was presented as if it were the strategy's whole record.

There was a second reason live/paper data never appeared: executed positions do carry the selector's choice, but it lives inside the stored order payload (`metadata_json → candidate → strategy_id`) rather than in a column, so nothing was reading it.

**Now.** `?source=paper|live|replay|all` (default `all`). The dashboard has a **Results from** selector:

- **Compare all three side by side** — one column per mode, each showing trades · win rate, with profit factor and net P&L beneath
- **Paper only** / **Live only** / **Replay only** — the full detailed columns for that one mode

The three are never averaged. The same strategy can be profitable in replay and lose money live, and a blended number would hide exactly that. Win-rate and profit-factor ratios stay hidden until a mode has at least `MIN_TRADES_FOR_EDGE_DISPLAY` (30) trades in that mode — a 75% win rate off four trades is noise, and showing it is how a system talks itself into trusting noise.

MFE/MAE and failure-type columns stay empty for paper/live rather than being estimated: they are only reconstructable from tick-by-tick replay, and putting fabricated numbers next to measured ones would be worse than leaving them blank. The Recurring failure patterns panel is replay-only for the same reason, and now states how many executed trades in the window it could not classify.

The **live selector's** evidence base is unchanged — it still uses the replay record (`selector_evidence_source: replay` in the response). Only the reporting changed, not the trading behaviour.

The intraday expectancy card now also carries a `by_mode` split, since paper fills have no real slippage, queue position or partial fills, and pooling them with live results lets one flatter the other.

## New environment variables

| Variable | Default | Purpose |
|---|---|---|
| `INTRADAY_MAX_OPEN_POSITIONS` | `50` | Default max concurrent intraday positions (0 = unlimited) |
| `INTRADAY_POSITION_LIMIT_FLOOR` | `50` | One-time floor applied to existing installs |
| `INTRADAY_POSITION_LIMIT_MAX` | `1000` | Hard ceiling accepted from UI/API |
| `INTRADAY_REPLAY_MAX_OPEN_POSITIONS` | `0` | Replay default (0 = unlimited) |
| `INTRADAY_MAX_TRADES_PER_DAY` | `50` | Raised from 3 |

## API summary

| Endpoint | Change |
|---|---|
| `GET /api/capital-report` | **New.** `source`, `start`, `end`, `capital`, `market`, `run_id`, `engine` |
| `GET /api/intraday/strategies` | **New** `source` param; adds `by_source` per strategy. Flat fields retained for backwards compatibility |
| `GET /api/intraday/failure-patterns` | Adds `source`, `market`; reports `executed_trade_counts` |
| `POST /api/intraday/settings` | Accepts `max_open_positions`; `max_trades_per_day` ceiling raised to 1000 |
| `POST /api/intraday-replay/start` | Accepts `max_open_positions` (`unlimited` \| `live` \| number) and `enforce_capital` |
| `GET /api/intraday-replay/<id>/status` | Adds `config` (limit, capital guard, capital) and `stats.capital` |

## Migration

No manual steps. The schema changes and the one-time floor migration run automatically in `init_db()` on first start. Existing settings, trades, replay runs and history are untouched.

**Worth checking after deploying:** open the Intraday Engine page and confirm **Max intraday positions** reads 50 (or set it to 0 for unlimited). If your capital is small, the position limit may not be what actually stops entries — the per-trade minimum position value and the cost gates will bind first, and the Capital & P&L breakdown's "peak money in use" figure will show you how much was really being deployed.

## Historical market-data export (Export Data tab)

Downloads the **raw candles** (not trades, not Replay results) so the data can be analysed independently of the decision layer.

Pick a universe (Replay Lab universe / full NIFTY LargeMidcap 250 / NIFTY 100 / Midcap 150 / custom list), an interval (5m default, 1m, 15m) and dates (range or specific days), then **Build historical dataset**. It runs in the background; when done, download the ZIP:

```text
historical_data/<YYYY-MM-DD>/<SYMBOL>.csv   timestamp,open,high,low,close,volume,epoch
market/NIFTY.csv, market/INDIA_VIX.csv
sectors/NIFTYIT.csv, NIFTYAUTO.csv, ...
universe/large_midcap.csv                   symbol, exchange, cap_class (LARGE/MID), company, industry, ISIN, sector index file, data status
quality/data_quality.csv                    bars per symbol per day vs expected
quality/fetch_failures.csv                  API windows that failed after retries (= real data gaps)
manifest.json, README.txt
```

Integrity: native provider interval only (no resampling, so no candle is built from later ones); timestamps written as served (IST with `+05:30`, provider epoch alongside); duplicate window-boundary candles removed (keep last, same rule as the Replay loader); failed API windows are retried and then *recorded*, never hidden; only closed sessions are exported. `cap_class`/membership are today's NSE constituents, not point-in-time.

API: `POST /api/historical-export/estimate|start`, `GET /api/historical-export/jobs`, `GET /api/historical-export/<id>/status|download`, `POST /api/historical-export/<id>/cancel`.

Optional env vars: `HISTORICAL_EXPORT_MAX_LOOKBACK_DAYS` (150), `HISTORICAL_EXPORT_MAX_SPAN_DAYS` (130), `HISTORICAL_EXPORT_MAX_ROWS` (8,000,000), `HISTORICAL_EXPORT_WORKERS` (3), `HISTORICAL_EXPORT_WINDOW_RETRIES` (2), `HISTORICAL_EXPORT_KEEP` (3), `HISTORICAL_EXPORT_DIR`, `HISTORICAL_EXPORT_NIFTY100_URL`.

## V2 six-phase intraday engine

The application now includes a deterministic V2 research/decision layer shared from the common intraday candidate builder used by Live, Paper and Replay. V2 uses completed 5-minute signal logic, targets 1-minute execution when real 1-minute history is available, attaches the same regime/signal/edge audit fields in all three modes, and can enforce a frozen positive-edge model as a hard no-trade gate.

Important: the supplied 61-session historical bundle is 5-minute only and contains no usable historical NIFTY/VIX/sector-index series or point-in-time universe/sector membership. V2 therefore records a 5-minute compatibility data contract for this research run instead of fabricating missing data. The frozen model produced by the walk-forward research run currently has zero proven buckets, so the shipped V2 gate defaults to **enabled** and will return NO TRADE until a genuinely positive out-of-sample bucket is validated.

V2 status endpoint: `/api/engine-v2/status`.

Official master-data references used by the research package are recorded in `data/master_data_sources.json` (NSE historical index data, India VIX, LargeMidcap 250, corporate actions and exchange data/report pages).

The research artifact is stored in `research/v2_simulation_report.json`; it is a validation report, not a claim of profitability.


## V3.2.2 Multi-Source Historical Data Export

V3.2.2 keeps INDstocks as the primary market-data/execution provider but no longer treats a rejected
INDstocks index code or a blocked public website as the end of the export.

Provider chain:

1. **INDstocks primary** — stock OHLCV and index OHLCV using only the documented REST scrip-code
   format `SEGMENT_TOKEN` (for example `NSE_3045`). The exporter no longer retries undocumented
   `IDX_...`, `INDEX_...`, or bare-token variants on the historical endpoint.
2. **Upstox V3 fallback** — exact `instrument_key` values from the official NSE/global JSON masters,
   including the documented `NSE_INDEX|India VIX`, sector indices, missing equity days via
   `NSE_EQ|<ISIN>`, GIFT NIFTY/global instruments, corporate actions, company profiles, FII/DII,
   and limited recent news/fundamentals where the official API actually provides them.
3. **Dhan V2 fallback for indices** — exact security IDs resolved from Dhan's official public
   instrument master, requested with `exchangeSegment=IDX_I` and `instrument=INDEX`.

Optional deployment secrets (never put these inside an exported ZIP):
...
INDstocks credentials remain the primary credentials already used by the application.

Every export now contains provider diagnostics such as:

```text
reference/index_probe.csv
reference/fallback_provider_attempts.csv
reference/provider_capabilities.csv
quality/dataset_status.csv
quality/date_completeness.csv
quality/data_anomalies.csv
```

Statuses distinguish `SUCCESS`, `EMPTY`, `FAILED`, `CREDENTIAL_MISSING`, `NOT_AVAILABLE`, and
provider-specific fallback results. Missing data should therefore never look like a successful empty export.

Additional export hygiene fixes:

- exact duplicated stock `15:30` rows are removed when they duplicate `15:29`;
- negative volume is recorded as an anomaly and sanitized before volume-based research;
- date-level symbol coverage is exported and partial sessions are marked non-research-eligible;
- missing stock-days can be backfilled through Upstox without overwriting successful INDstocks day files;
- NIFTY Financial Services alias resolution now includes the provider's `Nifty Financial` naming;
- Upstox standard API calls are paced under the documented minute rate ceiling and `429` responses are retried conservatively.

Historical top-of-book, order-book/tick replay, true historical pre-open auction history, and genuine
point-in-time universe/sector membership are still not fabricated when no connected historical source exists.
For live/paper operation the engine should collect quote/depth/fill telemetry going forward.

## V3.2.4 Low-Memory Historical Export

The 1-minute historical exporter now defaults to a Render-safe low-memory mode after an observed OOM restart:

- one 1-minute scrip per INDstocks historical request (`HISTORICAL_EXPORT_1M_SCRIPS_PER_CALL=1`);
- one 1-minute request in flight (`HISTORICAL_EXPORT_WORKERS=1` by default for 1m);
- memory is checked **before** every provider fetch, not only after a response is parsed;
- 128 MB response headroom is reserved by default (`HISTORICAL_EXPORT_MEMORY_RESERVE_MB`);
- Python GC plus glibc `malloc_trim(0)` is used between windows where available;
- PostgreSQL archive chunks default to 1 MB to avoid a large persistence spike;
- if safe RAM headroom is unavailable, the export refuses/pauses cleanly instead of allowing Render to OOM-kill the web service.

This intentionally trades speed for reliability. The compact-candle export and PostgreSQL completed-export persistence from V3.2.3 remain enabled.
